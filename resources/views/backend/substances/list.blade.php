    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card-box table-responsive">
                <h4 class="header-title m-t-0 m-b-30">Liste des Substances eexploitées</h4>
                <div class="row">
                    <div class="col-sm-6">
                        <div class="m-b-30">
                            <a id="add-substance" role="button" href="{{route('substanceexploitees.create')}}" class="btn btn-primary waves-effect waves-light">
                              AJOUTER SUBSTANCE <i class="fa fa-plus"></i>
                            </a>
                        </div>
                    </div>
                </div>

                <table id="substanceexploitees-table" class="table table-striped table-bordered dt-responsive nowrap"
                       cellspacing="0"
                       width="100%">
                    <thead>
                    <tr>
                        <th>{{ Lang::get('contenu.substance.nom')}}</th>
                        <th>{{ Lang::get('contenu.substance.description')}}</th>
                        <th>{{ Lang::get('contenu.substance.action')}}</th>
                    </tr>
                    </thead>
                </table>
...
@section('js')
    ...
    <script>
        $(document).ready(function () {
            var table = $('#substanceexploitees-table')
                .DataTable({
                    "oLanguage": {
                    ...
                    processing: true,
                    serverSide: true,
                    ajax: '{!! route('substanceexploitees.data') !!}',
            ...
            $(document).on('click', '.delete', function () {
                ...
                var url = '{{ route("substanceexploitees.delete", ":id") }}';
